...
# Function to display password securely with hover effect
def display_password_with_hover(password):
    """Displays a password as dots and reveals it on hover."""
    ...
    st.markdown("""
        <style>
            ...
            .password-box:hover {
                border-color: #888; /* Slightly darker grey on hover */
            }
            ...
            .password-box:hover::before {
                content: attr(data-password); /* Show actual password on hover */
            }
        </style>
    """, unsafe_allow_html=True)

    st.markdown(f'<div class="password-box" data-password="{password}"></div>', unsafe_allow_html=True)
...
def masked_password_display(password):
    """Displays a password with dots and reveals it on hover (Alternative Version)."""
    ...
    st.markdown("""
        <style>
            .masked-password {
                ...
                margin: 5px auto;
                padding: 8px 15px;
                border: 2px dashed #bbb; /* Dashed grey border for distinction */
                border-radius: 8px;
            ...
            .masked-password::before {
                content: '**********'; /* Hidden by default */
            }

            .masked-password:hover::before {
                content: attr(data-password); /* Revealed on hover */
            }
        </style>
    """, unsafe_allow_html=True)

    st.markdown(f'<div class="masked-password" data-password="{password}"></div>', unsafe_allow_html=True)
...
def simulate_cracking_animation(password):
    ...
    with placeholder.container():
        ...
        display_password_with_hover(password)
...
if len(password) < 8 or " " in password:
    st.error("🚨 Password must be at least 8 characters long and should not contain spaces!")
else:    
    ...
    with col1:
        ...
        for pwd, score in zip(passwords, scores):
            c1, c2 = st.columns([3, 1])  # 3:1 ratio for spacing

            with c1:
                masked_password_display(pwd)  # New function name

            with c2:
                st.markdown(f"<br> <p style='font-size:18px; font-weight:bold; text-align:right;'>Score: {score}/10</p>", unsafe_allow_html=True)
